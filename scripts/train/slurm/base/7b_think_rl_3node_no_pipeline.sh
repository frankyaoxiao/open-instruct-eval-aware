#!/bin/bash
#SBATCH --job-name=olmo3-7b-rl-3n-nopipe
...
# ===========================================================================
# OLMo 3 7B Think RL (GRPO) — 3-node setup, NO PIPELINE
#
# Matches the official released OLMo-3-7B-Think training config:
#   - inflight_updates=false (synchronous updates)
#   - async_steps=1
#   - No advantage normalization or IS ratio cap
#
# Node layout (same as pipeline variant):
...
RUN_NAME="olmo3-7b-think-rl-3node-no-pipeline"
OUTPUT_DIR="/data/artifacts/frank/openinstruct/olmo3-7b-think-rl-base-no-pipeline"
...
srun --overlap --nodes=1 --ntasks=1 -w "${HEAD_NODE}" bash -c "
    ...
    # --- Launch GRPO training (NO PIPELINE) ---
    echo '[HEAD] Starting GRPO training (no pipeline)...'
    set -e
